# Hi, I'm Hamesh Tiwari 👋

### **Senior Data Architect | AI Security & Infrastructure**
*15+ Years of Engineering High-Availability Systems & Scalable Data Foundations*

---

### **🚀 About Me**
I am a seasoned Data Architect transitioning legacy enterprise ecosystems into modern, distributed, and secure AI-driven architectures. My work focuses on the intersection of **Big Data Engineering** and **Generative AI Governance**. Currently pursuing my **MS in Computer Science at Woolf University**, specializing in secure RAG implementations and large-scale data orchestration.

- 🏗️ **Currently Building:** A Secure RAG (Retrieval-Augmented Generation) Auditor for enterprise financial data.
- 🧪 **Specializing In:** Neutralizing LLM vulnerabilities (OWASP Top 10 for LLM) through metadata-driven SQL shielding.
- 🏃 **Endurance Athlete:** Multiple **Full Ironman Triathlons** and **50K Ultra-marathons** completed. I apply the same "long-game" discipline to architectural stability and system performance.

---

### **🛠️ Tech Stack & Expertise**
- **Languages:** Python (PySpark), Java, SQL (Hive/Presto/Oracle), Bash.
- **Big Data:** Spark 3.x, Hadoop, Hive LLAP, Kafka, Apache Airflow.
- **AI & Security:** LangChain, RAG Architectures, SQL Injection Defense, PII Masking, Gemini/OpenAI API.
- **Infrastructure:** AWS (S3, EC2), DataBricks , Redis, Docker, CI/CD (GitHub Actions).

---

### **📊 Featured Projects** (Updating)
- **[Secure-AI-Auditor](https://github.com/HameshTiwari/your-repo-link):** A secure RAG gateway with a custom SQL Shield to prevent prompt injection and unauthorized DDL operations.
- **[Retail-Medallion-Pipeline](https://github.com/HameshTiwari/your-repo-link):** An end-to-end Spark/Airflow pipeline processing 50M+ records using Parquet/Snappy for optimized OLAP analytics.

---

### **📫 Connect with Me**
- **LinkedIn:** [Hamesh Tiwari]
- **Interests:** Animal Welfare (Rescue Advocate), Classical Music Structure, Precision Horticulture.

---
*“In both data architecture and ultra-endurance sports, success is built on resilience, precision, and a relentless focus on the finish line.”*
